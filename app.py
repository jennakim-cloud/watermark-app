...
def load_logo(brand: str, color: str, final_w: int, final_h: int) -> Image.Image:
    """4배 PNG 로고를 로드해 최종 크기로 다운샘플"""
    info = LOGO_FILES.get(brand, {})
    path = info.get(color)
    if path and path.exists():
        logo = Image.open(path).convert("RGBA")
        return logo.resize((final_w, final_h), Image.LANCZOS)
...
def apply_watermark(
    ...
    info = LOGO_FILES.get(brand, {})
    dim, target = info.get("dim", "width"), info.get("size", 126)
    # 저장된 PNG의 가로세로 비율로 최종 크기 계산
    tmp_path = info.get(color)
    ...
    logo = load_logo(brand, color, final_w, final_h)
